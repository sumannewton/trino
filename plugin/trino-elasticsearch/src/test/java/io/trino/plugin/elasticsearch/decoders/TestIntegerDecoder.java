...
public class TestIntegerDecoder
        extends AbstractDecoderTest
{
    @Override
    protected String getParseFailureErrorMsg(Object value)
    {
        return format("Cannot parse value for field '%s' as INTEGER: %s", PATH, value);
    }

    @Override
    protected String getTypeMismatchErrorMsg(Object value)
    {
        return format("Expected a numeric value for field '%s' of type INTEGER: %s [%s]", PATH, value, value.getClass().getSimpleName());
    }
    ...
    public void testOutOfRangeLongValue()
    {
        ...
        assertTrinoExceptionThrownBy(() -> decoder.decode(PATH, valueSupplier, output),
                format("Value out of range for field '%s' of type INTEGER: %s", PATH, value));
    ...
    {
        // positive double
        Double value = 123456789123456.123;
        when(valueSupplier.get()).thenReturn(value);
        assertTrinoExceptionThrownBy(() -> decoder.decode(PATH, valueSupplier, output),
                format("Value out of range for field '%s' of type INTEGER: %s", PATH, value.longValue()));
    ...
    {
        // positive float
        Float value = 12345678912345.123f;
        when(valueSupplier.get()).thenReturn(value);
        assertTrinoExceptionThrownBy(() -> decoder.decode(PATH, valueSupplier, output),
                format("Value out of range for field '%s' of type INTEGER: %s", PATH, value.longValue()));

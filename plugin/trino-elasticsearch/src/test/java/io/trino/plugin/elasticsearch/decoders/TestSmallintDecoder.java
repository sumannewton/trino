...
public class TestSmallintDecoder
        extends AbstractDecoderTest
{
    @Override
    protected String getParseFailureErrorMsg(Object value)
    {
        return format("Cannot parse value for field '%s' as SMALLINT: %s", PATH, value);
    }

    @Override
    protected String getTypeMismatchErrorMsg(Object value)
    {
        return format("Expected a numeric value for field '%s' of type SMALLINT: %s [%s]", PATH, value, value.getClass().getSimpleName());
    }

    @BeforeClass
    public void setUp()
    {
        decoder = new SmallintDecoder();
        init();
    }
    ...
    public void testOutOfRangeIntegerValue()
    {
        ...
        assertTrinoExceptionThrownBy(() -> decoder.decode(PATH, valueSupplier, output),
                format("Value out of range for field '%s' of type SMALLINT: %s", PATH, value));
    ...
    public void testOutOfRangeDoubleValue()
    {
        ...
        when(valueSupplier.get()).thenReturn(value);
        assertTrinoExceptionThrownBy(() -> decoder.decode(PATH, valueSupplier, output),
                format("Value out of range for field '%s' of type SMALLINT: %s", PATH, value.longValue()));
